...
MODEL_OUTPUT_PATH = os.path.join("data", "processed", "lstm_v3_clean.pt")
SCALER_OUTPUT_PATH = os.path.join("data", "processed", "lstm_v3_scaler.pkl")

# Documented Reference Benchmark Values (LSTM v1)
LSTM_V1_ACC = 0.6806
LSTM_V1_F1 = 0.5306
LSTM_V1_FPR = 0.0182
...
# =====================================================================
# STANDALONE V3 MODEL DEFINITION (2-LAYER STACKED + DROPOUT)
# =====================================================================

class StandaloneLSTMv3ForecastModel(nn.Module):
    """
    PyTorch LSTM Classifier for Next-State Network Attack Forecasting (v3 Capacity Experiment).

    Architecture Changes vs v1:
    - Input size: 15
    - Hidden size: 128 (Increased from 64)
    - Layers: 2 (Increased from 1)
    - Dropout: 0.2 (Between stacked LSTM layers)
    - Batch first: True
    - Takes final timestep -> Linear(128, 2)
    ...
    def __init__(
        ...
        super(StandaloneLSTMv3ForecastModel, self).__init__()
...
# =====================================================================
# UTILITY HELPERS & PREPROCESSING (STANDARDSCALER ONLY)
# =====================================================================
...
def evaluate_model(model: nn.Module, data_loader: DataLoader, device: torch.device):
    """Evaluates PyTorch model on a DataLoader using standard argmax predictions."""
    model.eval()
    all_preds = []
    all_targets = []
    total_loss = 0.0
...
def run_experiment_lstm_v3():
    """Execution entry point for LSTM v3 (Hidden 128, 2 Layers, Dropout 0.2)."""
    # 1. Reproducibility & Device Initialization
    set_reproducibility_seeds(42)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print("==================================================")
    print("      LSTM v3 — CAPACITY EXPERIMENT (HIDDEN 128)  ")
    print("==================================================")
    ...
    model = StandaloneLSTMv3ForecastModel(
        input_size=15, hidden_size=128, num_layers=2, num_classes=2, dropout_prob=0.2
    ).to(device)
    print("\nModel Architecture:")
    print(model)
    ...
    best_model = StandaloneLSTMv3ForecastModel(
        input_size=15, hidden_size=128, num_layers=2, num_classes=2, dropout_prob=0.2
    ).to(device)
    ...
    print("========================================================================")
    print(f"{'Experiment':<32} | {'Accuracy':<10} | {'F1 Score':<10} | {'FPR':<10}")
    print("-" * 72)
    print(f"{'LSTM v1 (StandardScaler Only)':<32} | {LSTM_V1_ACC*100:>9.2f}% | {LSTM_V1_F1*100:>9.2f}% | {LSTM_V1_FPR*100:>9.2f}%")
    print(f"{'LSTM v3 (Hidden 128, 2-Layer)':<32} | {test_acc*100:>9.2f}% | {test_f1*100:>9.2f}% | {test_fpr*100:>9.2f}%")
    print("========================================================================\n")


if __name__ == "__main__":
    run_experiment_lstm_v3()